...
class customized_order(models.Model):
	...
	def _default_template(self):
	    ...
	    if not company.template_order:
		def_tpl = self.env['ir.ui.view'].search([('key', 'like', 'professional_templates.template_%' ), ('type', '=', 'qweb')], order='id asc', limit=1)
                company.write({'template_order': def_tpl.id})
	    return company.template_order or self.env.ref('sale.report_saleorder_document')
	...
	theme_color = fields.Char('Theme Color', size=7, required=True, default=_default_theme_color, help="The Main Theme color of the sale order/quaotation. Normally this\
